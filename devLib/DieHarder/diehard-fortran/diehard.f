c************top of file
      ...
      character*80 text(36),dum
      integer which(15)
      open(4,file='tests.txt')
      read(4,766) (dum,j=1,244)
      read(4,766) (text(j),j=1,15)
      print 766,(text(j),j=1,15 )
 766  format(a78)
      close(4)
 1    print*,' Enter filename (<=15 characters):'
      read 845, filename
 845  format(a15)
      open(1,file=filename,form='unformatted',access='direct',
     & recl=16384)
       print*,' Enter name of output file (<=15 characters):'
      read 845,fileout
      open(3,file=fileout)
      print 766,(text(j),j=1,14 )
      write(3,766)(text(j),j=1,14 )
      print*,' Which tests do you want performed?'
      print*,' For all tests, enter 15 1`s:'
      print*,' 111111111111111'
      print*,' For, say, tests 1,3,7 and 14, enter'
      print*,' 101000100000010'
      print*,'     HERE ARE YOUR CHOICES:'
      print*,'     1  Birthday Spacings'
      print*,'     2  Overlapping Permutations'
      print*,'     3  Ranks of 31x31 and 32x32 matrices'
      print*,'     4  Ranks of 6x8 Matrices'
      print*,'     5  Monkey Tests on 20-bit Words'
      print*,'     6  Monkey Tests OPSO,OQSO,DNA'
      print*,'     7  Count the 1`s in a Stream of Bytes'
      print*,'     8  Count the 1`s in Specific Bytes'
      print*,'     9  Parking Lot Test'
      print*,'    10  Minimum Distance Test'
      print*,'    11  Random Spheres Test'
      print*,'    12  The Sqeeze Test'
      print*,'    13  Overlapping Sums Test'
      print*,'    14  Runs Test'
      print*,'    15  The Craps Test'
      Print*,' Enter your choices, 1`s yes, 0`s no. using 15 columns:'
 158  format(a)
      print 158,'123456789012345'
      read 876,which
 876  format(15i1)
      if(which(1).eq.1) call cdbday(filename)
      if(which(2).eq.1) call cdoperm5(filename)
      if(which(3).eq.1) call rank3132(filename)
      if(which(4).eq.1) call cdbinrnk(filename)
      if(which(5).eq.1) call cdbitst(filename)
      if(which(6).eq.1) call cdomso(filename)
      if(which(7).eq.1) call sknt1s(filename)
      if(which(8).eq.1) call wknt1s(filename)
      if(which(9).eq.1) call cdpark(filename)
      if(which(10).eq.1) call mindist(filename)
      if(which(11).eq.1) call d3sphere(filename)
      if(which(12).eq.1) call sqeez(filename)
      if(which(13).eq.1) call cdosum(filename)
      if(which(14).eq.1) call runtest(filename)
      if(which(15).eq.1) call craptest(filename)
      print 472,fileout
      write(3,472) fileout
...
C     If n=2^20, should be 385750 missing 20-letter words, sigma   512
      ...
      jkk=jkreset()
      open(4,file='tests.txt')
      read(4,766) (dum,j=1,61)
      read(4,766) (text(j),j=1,16)
      print 766,(text(j),j=1,16)
      write(3,766)(text(j),j=1,16)
 766  format(a78)
      close(4)
      n=20
      kpow=21
...
C     ****END OF MAIN LOOP****
 ...
 489  format(/,'$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$',/)
      ...
      n=4000
      open(4,file='tests.txt')
      read(4,766) (dum,j=1,186)
      read(4,766) (text(j),j=1,12)
      print 766,(text(j),j=1,12)
      write(3,766) (text(j),j=1,12 )
 766  format(a78)
      close(4)
      jkk=jkreset()
...
c Put a one-line function here to provide the uni being tested:
      uni()=.5+jtbl()*.5**32
       open(4,file='tests.txt')
      jkk=jkreset()
       read(4,766) (dum,j=1,198)
       read(4,766) (text(j),j=1,10)
       print 766,(text(j),j=1,10)
       write(3,766) (text(j),j=1,10)
766    format(a78)
       close(4)
1      do 7 i=6,48
7      tbl(i)=0
...
489   format(/,'$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$',/)
      ...
      ss=0.
       open(4,file='tests.txt')
       read(4,766) (dum,j=1,151)
       read(4,766) (text(j),j=1,22)
       print 766,(text(j),j=1,22)
       write(3,766) (text(j),j=1,22)
766    format(a78)
       close(4)
...
c***** one line function to generate a random coordinate in [0,1000).
      uni()=5000.+jtbl()*.2328306e-5
       open(4,file='tests.txt')
       read(4,766) (dum,j=1,173)
       read(4,766) (text(j),j=1,13)
       print 766,(text(j),j=1,13)
       write(3,766) (text(j),j=1,13)
766    format(a78)
       close(4)
      n=8000
      ns=100
...
C     Calculate the test statistics uv and dv.
 ...
 700  return
       ...
       integer nt(21)
       character*15 filename
       character*80 text(12)
       parameter(cc=6.*.5**32)
       kthrow()=2+int(cc*jtbl()+3.)+int(cc*jtbl()+3.)
       jkk=jkreset()
       open(4,file='tests.txt')
       read(4,766) (dum,j=1,232)
       read(4,766) (text(j),j=1,12)
       print 766,(text(j),j=1,12)
       write(3,766) (text(j),j=1,12)
766    format(a78)
...
879               j=69069*j
       open(4,file='tests.txt')
       read(4,766) (dum,j=1,77)
       read(4,766) (text(j),j=1,36)
       print 766,(text(j),j=1,36)
       write(3,766) (text(j),j=1,36)
766    format(a78)
       close(4)
       do 2001 jk=1,3
       k=2*jk
...
c******that is, this function provides a random keystoke, unequal p's.
        ...
        n=10
       open(4,file='tests.txt')
       read(4,766) (dum,j=1,131)
       read(4,766) (text(j),j=1,20)
       print 766,(text(j),j=1,20)
       write(3,766) (text(j),j=1,20)
766    format(a78)
       close(4)
      print 2345, filename
2345   format('   Test results for ',a15)
...
c************ one-line function to get random byte:*************
       ...
       open(4,file='tests.txt')
       read(4,766) (dum,j=1,51)
       read(4,766) (text(j),j=1,10)
       print 766,(text(j),j=1,10)
       write(3,766) (text(j),j=1,10)
766    format(a78)
       close(4)
c       open(3,file='testout')
       write(3,37) filename
       print 37,filename
...
cc**** Finds binary rank of m rows, n trailing bits each**********
      ...
      data p/.2887880952,.5775761902,.1283502644,.0052854502/
       open(4,file='tests.txt')
       read(4,766) (dum,j=1,33)
       read(4,766) (text(j),j=1,18)
766    format(a78)
       close(4)
11       do 999 m=31,32
       jkk=jkreset()
       if(m.eq.31)   print 766,(text(j),j=1,9)
       if(m.eq.31) write(3,766) (text(j),j=1,9)
       if(m.eq.32)  print 766,(text(j),j=10,18)
       if(m.eq.32)   write(3,766) (text(j),j=10,18)
       n=m
       print 2345,filename
       write(3,2345) filename
...
212    format(8i10)
       open(4,file='tests.txt')
       read(4,766) (dum,j=1,18)
       read(4,766) (text(j),j=1,15)
       print 766,(text(j),j=1,15)
       write(3,766) (text(j),j=1,15)
766    format(a78)
       close(4)
          DO 8888 ijkm=1,2
       do 2 i=1,59
...
c  32  format(' UNI = ',f8.5,' (Probability of a better fit.)')
     ...
     &.8980,.9095,.9214,.9337,.9464,.9595,.9731,.9868,.9983, 1./
      ...
      jkk=jkreset()
       open(4,file='tests.txt')
       read(4,766) (dum,j=1,208)
       read(4,766) (text(j),j=1,11)
       print 766,(text(j),j=1,11)
       write(3,766) (text(j),j=1,11)
766    format(a78)
       close(4)
11       do 89       ik=1,10
      qmax=0
